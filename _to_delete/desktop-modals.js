/* Modal dùng chung cho desktop shell: Cài đặt (2 cột) + Thêm bạn.
   Nhúng: <script src="desktop-modals.js">. Gọi openSettingsModal() / openAddFriendModal(). */
(function(){
  ...
  var SET = {
    general:
      '<div class="set-sec"><h4>Danh bạ</h4><div class="desc">Danh sách bạn bè được hiển thị trong danh bạ</div>'+
        '<div class="set-card">'+
          '<div class="set-row"><span class="lbl">Hiển thị tất cả bạn bè</span><span class="radio2" onclick="setRadio(this)"></span></div>'+
          '<div class="set-row"><span class="lbl">Chỉ hiển thị bạn bè đang hoạt động</span><span class="radio2 on" onclick="setRadio(this)"></span></div>'+
        '</div></div>'+
      '<div class="set-sec"><h4>Giao diện và ngôn ngữ</h4>'+
        ...
        '</div></div>'+
      '<div class="set-sec"><h4>Khởi động &amp; ghi nhớ tài khoản</h4>'+
        '<div class="set-card">'+
          '<div class="set-row"><span class="lbl">Khởi động khi mở máy</span><span class="switch" onclick="this.classList.toggle(\'on\')"></span></div>'+
          '<div class="set-row"><span class="lbl">Ghi nhớ tài khoản đăng nhập</span><span class="switch on" onclick="this.classList.toggle(\'on\')"></span></div>'+
        '</div></div>',
    account:
    ...
    about:
      ...
      '<div class="set-sec"><div class="set-card">'+
          ...
          '<div class="set-row"><span class="lbl">Chính sách quyền riêng tư</span><span class="link" onclick="location.href=\'terms.html\'">›</span></div>'+
          '<div class="set-row"><span class="lbl">Hỗ trợ</span><span class="link">›</span></div>'+
        '</div></div>'
  };
  var MENU = [
  ...
  window.openFriendModal=function(f){
    ...
    var av=document.getElementById('frAv'); av.textContent=f.av||name[0]; av.style.background=f.color||'#5B6470';
  ...
  window.setRadio=function(el){ el.parentNode.parentNode.querySelectorAll('.radio2').forEach(function(r){ r.classList.remove('on'); }); el.classList.add('on'); };
})();
